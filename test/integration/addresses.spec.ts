...
describe('bitcoinjs-lib (addresses)', () => {
  ...
  it('can generate a Guapcoin address', () => {
    // WARNING: although possible, bitcoinjs is NOT necessarily compatible with Guapcoin
    const GUAPCOIN = {
    ...
    const keyPair = bitcoin.ECPair.makeRandom({ network: GUAPCOIN });
    const { address } = bitcoin.payments.p2pkh({
      pubkey: keyPair.publicKey,
      network: GUAPCOIN,
    });
